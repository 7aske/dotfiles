...
# based on https://gitlab.com/aquator/i3-scratchpad

print_help() {
	printf 'Usage: %s [[-a <anchor>] [-d <size>] [-p <pos>] [-s <screen>]] [-m <edge>] [-t] [-u [-w][-o <opts>]] [-v] [-V] <command>\n' "$script_name"
	printf 'Executes a program in a positioned scratchpad, optionally wrapped in a terminal window.\n'
	printf 'Caches optional terminal wrapper script and stores window id at XDG_RUNTIME_DIR based on command,\n'
	...
	printf ' -o <opts>     Extra terminal options to pass.\n'
...
main() {
	local screen_width screen_height offset_x offset_y width height x_pos y_pos \
	      terminal move wait command terminal_opts toggle verbose fullscreen \
...
parse_aruments() {
    ...
    toggle=true
	while getopts a:d:fhm:o:p:s:tTvVw arg; do
		case $arg in
			a) anchor="$OPTARG";;
...
show_window_in_position() {
	...
	if [ -z "$move" ]; then
        ...
        i3-msg -q -- "[id=\"$window_id\"] move to scratchpad;[id=\"$window_id\"] scratchpad show;[id=\"$window_id\"] move position $x_pos px $y_pos px;[id=\"$window_id\"] resize set $width px $height px$([ -n "$fullscreen" ] && printf ";[id=\"$window_id\"] fullscreen enable")"
		return 0
	fi
	...
	i3-msg -q -- "[id=\"$window_id\"] move position $x_pos px $y_pos px$([ -n "$fullscreen" ] && printf ";[id=\"$window_id\"] fullscreen enable")"
}
...
create_terminal_wrapper() {
		...
		if [ -f "$wrapper_file" ] && [ -x "$wrapper_file" ]; then
			...
			debug 'Creating wrapper file' "$wrapper_file" 'with terminal command wrapper'
			printf '#!/bin/bash\n' > "$wrapper_file"
			printf 'clear\n' >> "$wrapper_file"
		...
		command="$TERMINAL $terminal_opts -e '$wrapper_file'"
}
